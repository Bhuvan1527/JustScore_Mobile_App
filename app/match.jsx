...
import addOrUpdateMatch from '../services/StoreMatch'
...
const Match = () => {
  ...
  useEffect(() => {
    if (metaData.run_for_wide === 'yes') {
      ...
      setMatchDatabase(prevState => ({...prevState, wideRuns : metaData.run_for_wide}));
    }
    return;
  }, [])
  ...
  useEffect(() => {
    if (!isSecondInnings && !isFirstInnings) {
      // router.replace('/home')
      if(matchSeq.currentScore < Target){
        setMatchDatabase(prev => ({...prev, Winner : 'Team A'}));
      }
      if (matchSeq.currentScore > Target) {
        setMatchDatabase(prev => ({...prev, Winner : 'Team B'}));
      }
      if (matchSeq.currentScore == Target) {
        setMatchDatabase(prev => ({...prev, Winner : 'DRAW'}));
      }
      console.log(MatchDatabase); 
      addOrUpdateMatch(user.uid, MatchDatabase);
    }
    if (isSecondInnings) {
  ...
  const updateMatchDatabase = (over, ballData) => {
    if (isFirstInnings) {
      setMatchDatabase(prevState => ({
        ...prevState,
        firstInnings: {
          ...prevState.firstInnings,
          [over]: prevState.firstInnings[over]
            ? [...prevState.firstInnings[over], ballData]  // If over exists, add ball
            : [ballData]  // If over doesn't exist, create new array
        }
      }));
    }
    if (isSecondInnings) {
      setMatchDatabase(prevState => ({
        ...prevState,
        secondInnings: {
          ...prevState.secondInnings,
          [over]: prevState.secondInnings[over]
            ? [...prevState.secondInnings[over], ballData]  // If over exists, add ball
            : [ballData]  // If over doesn't exist, create new array
        }
      }));
    }
  ...
  const handleScore = (runs, wickets) => {
    ...
    if (wickets != 0) {
      ...
      updateMatchDatabase(matchSeq.currentOver, 'wicket');
      if (matchSeq.currentWicketsDown + wickets >= metaData.no_of_players_per_team) {
        if (isFirstInnings) {
          updateMatchDatabase('TotalScore', matchSeq.currentScore);
      ...
      updateMatchDatabase(matchSeq.currentOver, runs);
    }
    setMatchSeq(prevMatchSeq => ({
      ...prevMatchSeq,
  ...
  const handleWide = () => {
    ...
    updateMatchDatabase(matchSeq.currentOver, 'wide');
  }
